...
# PRE-PROCESSING / DATA MUNGING
    ...
    raw.data.cluster <- aggregate(raw.data.cluster, by = list(raw.data.cluster$State), FUN = mean, na.rm = T)
    rownames(raw.data.cluster) <- raw.data.cluster$Group.1

    # Convert Data to Numeric-Only For Clustering
    colnames(raw.data.cluster[,c(1,2,3,4)])
    str(raw.data.cluster[,c(-1,-2,-3,-4)])
    num.data.cluster <- raw.data.cluster[,c(-1,-2,-3,-4)]
...
# Cluster Plot #########
    ...
    fviz_cluster(list(data = num.data.cluster, cluster = sub_grp), 
                 main = 'Cluster Plot',
                 xlab = '',
                 ylab = '',
                 pointsize = num.data.cluster$PovertyRate)  
